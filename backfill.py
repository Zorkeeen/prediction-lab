...
def main():
    state = json.loads(STATE.read_text()) if STATE.exists() else {"offset": 0,
                                                                  "done": False}
    if state.get("done"):
    ...
    append_gz(OUT / f"hist_{start}.csv.gz", ["id", "t", "p"], hist_rows,
              force_header=True)
    state["offset"] = offset
    STATE.write_text(json.dumps(state))
    print(f"backfill: {n} markets (offset {start}->{offset}, "
          f"{len(hist_rows)} history points, done={state['done']})")
